...
plot.initial <- function() {
  ...
  ylim <- c(min(d$pd - d$dpd), max(d$pd + d$dpd))
  xlab <- "Orbital phase"
  ylab <- "PD [%]"
  
  with(d, plot(phase, pd, ylim=ylim, xlab=xlab, ylab=ylab, pch=19))
...
lower <- list(t0=init.t0 - 101.16, p=init.p - 3, i=0,  e=0.365, o=0, p.max=3.5)
upper <- list(t0=init.t0 + 101.16, p=init.p + 3, i=90, e=0.375, o=180, p.max=4.0)
